---
name: linear-chief-build
description: >
  Pull every Linear ticket in the "Todo" workflow status (optionally scoped
  to a team or assignee), then run /chief-build on each ticket
  ...
  "/linear-chief-build", "burn down my linear todos", "ship every linear
  todo", "chief-build every linear todo", or wants to autonomously work
  through a Linear backlog with one clean PR per issue.
---

# /linear-chief-build — Burn down a Linear "Todo" queue, one clean PR per ticket

You are a backlog runner. For every open Linear ticket in **Todo** status,
you launch `/chief-build` against that ticket's description and ship a
**separate PR** per ticket. Never bundle multiple tickets into one PR.
Every build must meet the clean-code bar in §3a.

## 0. Inputs

`/linear-chief-build [flags]`. Optional flags:
...
3. **`gh` available.** `gh auth status` must succeed — `/chief-build`
   needs it for PR creation.
4. **`/chief-build` reachable.** This skill invokes it via the Skill
...
If `--dry-run`, stop. Otherwise: "Run chief-build against all N
...
### 3a. Build the chief-build invocation

Hand `/chief-build` exactly this prompt body (one ticket only):
...
`/chief-build` adds its own timestamp suffix.
...
### 3c. Invoke `/chief-build` — ONE invocation per ticket

**This is the most-violated rule. Read carefully.**

Make **exactly one** Skill-tool call to `/chief-build` per ticket. Never
batch tickets into a single invocation. Never reuse a worktree across
tickets. If your prompt to `/chief-build` mentions two ticket IDs, you
...
Call `/chief-build` via the Skill tool with `args`:
```
--branch $TARGET
...
`/chief-build` runs end-to-end in that single turn (worktree, plan,
...
- `gh pr list` must now show **exactly one** new open PR vs.
  `PRS_BEFORE` whose head ref starts with `cb/<ticket-slug>-`.
...
Record: PR URL, PR number, branch name (must start with `cb/`),
...
`--parallel N > 1`:
- N ticket subroutines run concurrently. Each `/chief-build` produces
  its own worktree (`cb/...`), so they don't collide.
...
- /path/to/repo.cb-eng-130-...  (ENG-130, escalated)
